...
void vector_test(){
    Vect* vect = create_vect();
    for(int i = 0; i <= 50; i++){
        char car[2];
        car[0] = '!' + i;
        car[1] = '\0';
        Item* item = create_item("casa", 1, car, 200);
        add_to_vect(vect, item);
    }
    assert(vect->size == 51);
    for(int i = 0; i <= 50; i++)
...
void mod_item_repo_test(){
    ...
    assert(((Item*)repo->vect->items[0])->price == 10);
    assert(((Item*)repo->vect->items[0])->surface == 120);
    assert(str_equal(((Item*)repo->vect->items[0])->tip, "apartament"));
    destroy_repo(repo);
}
...
void mod_it_srv_test(){
    ...
    assert(((Item*)srv->repo->vect->items[0])->price == 0);
    assert(srv_mod_surface(srv, "adr", 0)==1); //ok
    assert(((Item*)srv->repo->vect->items[0])->surface == 0);
    assert(srv_mod_tip(srv, "adr", "apartament")==1); //ok
    assert(str_equal(((Item*)srv->repo->vect->items[0])->tip, "apartament"));
...
void filter_test(){
    ...
    assert(vect1->size==1);
    assert(str_equal(((Item*)vect1->items[0])->tip, "casa"));

    Vect* vect2 = type_filter(srv, "apartament");
    assert(vect2->size==1);
    assert(str_equal(((Item*)vect2->items[0])->tip, "apartament"));

    Vect* vect3 = type_filter(srv, "teren");
    assert(vect3->size==1);
    assert(str_equal(((Item*)vect3->items[0])->tip, "teren"));
